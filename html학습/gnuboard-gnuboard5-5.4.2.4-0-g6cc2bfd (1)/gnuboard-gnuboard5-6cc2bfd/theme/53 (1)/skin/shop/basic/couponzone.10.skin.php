    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        switch($row['cp_method']) {
            case '0':
                ...
                $cp_target = '<a href="'.shop_item_url($row3['it_id']).'" target="_blank">'.get_text($row3['it_name']).'</a>';
                break;
            case '1':
                $sql3 = " select ca_id, ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$row['cp_target']}' ";
                $row3 = sql_fetch($sql3);
                $cp_target = '<a href="'.shop_category_url($row3['ca_id']).'" target="_blank">'.get_text($row3['ca_name']).'</a>';
                break;
            case '2':
                $cp_target = '주문금액할인';
                break;
